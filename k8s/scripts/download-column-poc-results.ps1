...
.DESCRIPTION
  ...
  한 번 찍어 <실행타임스탬프>/ 폴더를 만들고, 그 아래는
  iter<회차>/<모델명>/<csv_stem>/ 으로 갈린다(CSV마다 타임스탬프가 따로 찍히지
  않는다). 회차는 Job의 ITERATIONS(같은 입력을 몇 번 반복하는지)이고 1이어도
  iter1/이 생긴다. LLM_MODEL에 모델을 여러 개 적었으면 회차 폴더 아래에 모델
  폴더가 모델 수만큼 생긴다 - 같은 실험의 같은 회차, 모델별 결과다. 각 <csv_stem>/ 안에는
  결과 문서 6개와 그 실행의 run.log가 같이 있다. 각 <csv_stem>/ 안에는 결과 문서
  6개(result.semantic.columns.json / .rulebase.json / .plan.json / .table.json /
  .llm_calls.json / .lean.json)와 run.log(성공/실패 모두 항상 남음)가 같이 들어있다. 중간에
...
.PARAMETER Target
  ...
  (예: 20260818_140500 - 그 실행 전체, .../iter1 - 1회차만,
  .../iter1/<모델명> - 그 회차의 그 모델만, .../iter1/<모델명>/my_asset - CSV 하나만).
  생략하면 전체를 받는다.
...
.EXAMPLE
  ...
  ./k8s/scripts/download-column-poc-results.ps1 -LocalDir .\results -Target 20260818_140500/iter1/Qwen-Qwen2.5-32B-Instruct/my_asset
#>
param(
