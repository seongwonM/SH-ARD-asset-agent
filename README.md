...
`requires`를 완결성으로 판정하는 이유: `board.has()`만 보면 컬럼 1개만 해석돼도
True가 되어, 5개 중 2개만 끝난 상태에서 최종 합성이 실행된다(실제로 겪은 버그).
단 완결을 영원히 기다릴 수는 없으므로, 후보가 고갈되면 2차 pass에서 부분 충족을 허용한다.
