...
description: 이 테이블의 한 행이 무엇을 의미하는지(행 단위)를 판정한다. 식별자와 시간축 컬럼의 해석 결과를 종합해 "장비별 공정 실행 1건" 같은 기록 단위를 도출한다. 컬럼 의미가 절반 이상 채워진 뒤에 실행된다.
requires: [table_profile]
provides: [grain]
applies_when:
  min_columns: 2
role: interpreter
capabilities: [grain_hypothesis]
cost: low
per_column: false
max_attempts: 2
---
